...
**J'ai exécuté l'étape 4 (« CASSER ») plutôt que de la croire — un des quatre résultats annoncés est faux.** Le paramètre manquant et la catégorie inconnue renvoient bien **400**, la route inconnue **404**, et l'API répond correctement (215 leçons, `date` et `slug` conformes). Mais la tentative de *path traversal* `curl 'http://localhost:3000/../../etc/passwd'`, annoncée comme devant retourner **403**, retourne **404** — y compris avec `--path-as-is`, donc ce n'est pas curl qui normalise. La cause est en amont : `new URL()` réduit le chemin à `/etc/passwd`, qui reste sous `DOSSIER_PUBLIC` et échoue à la lecture avant d'atteindre le garde-fou. **Le garde-fou de la ligne 238 est bon ; c'est le test proposé qui ne l'atteint pas.**
