---
type: fiche-document
source: 2026-08-15_lecon-placement-financier_10_fiscalite-revenus-capital.docx
date_creation: 2026-08-15
date_lecon: 2026-08-15
parcours: placement-financier
numero: 10
statut: parcours-actif
tags:
  - parcours/placement-financier
  - notion/pfu
  - notion/bareme-progressif
  - notion/prelevements-sociaux
  - notion/tmi
  - notion/csg-deductible
  - notion/erosion-rendement
  - enveloppe/pea
  - enveloppe/assurance-vie
---

# 2026-08-15_lecon-placement-financier_10_fiscalite-revenus-capital

Document source : [[2026-08-15_lecon-placement-financier_10_fiscalite-revenus-capital.docx]]

## Résumé

La leçon s'inscrit dans la continuité directe de la précédente : après les frais, **la fiscalité est le second levier d'érosion silencieuse du rendement réel**. Comprendre les deux permet d'arbitrer entre enveloppes sur le coût complet, et non sur le seul affichage commercial.

Deux régimes coexistent. Le **PFU** — la « flat tax » — s'applique par défaut, sans démarche : 12,8 % d'impôt sur le revenu à taux fixe, plus 18,6 % de prélèvements sociaux, soit **31,4 % en 2026**. La leçon signale explicitement le changement d'année : le PFU est passé de 30 % à 31,4 % au 1er janvier 2026, la CSG ayant été portée de 9,2 % à 10,6 % pour financer la branche autonomie — **toute source antérieure à janvier 2026 est donc périmée sur ce point**. En pratique, l'établissement prélève 12,8 % à la source comme acompte, régularisé à la déclaration ; une dispense est possible sous condition de revenu fiscal de référence (25 000 € pour un célibataire, 50 000 € pour un couple), à demander avant le 30 novembre de l'année précédente.

L'alternative est l'**option pour le barème progressif**, exercée en cochant la case 2OP. Trois caractéristiques la commandent : elle est **globale** — tous les revenus du capital de l'année, sans choix produit par produit ; les prélèvements sociaux restent dus à l'identique dans les deux régimes ; et elle ouvre deux avantages propres, l'**abattement de 40 % sur les dividendes** bruts de sociétés françaises ou européennes, et la **déductibilité de 6,8 % de CSG** sur le revenu imposable de l'année suivante. Nouveauté 2026 relevée par la leçon : **l'option n'est plus irrévocable**, elle se rejoue chaque année.

L'arbitrage se lit à la tranche marginale d'imposition. En dessous de 12,8 % de TMI — donc à 0 % ou 11 % — le barème est mécaniquement plus favorable ; à 41 % ou 45 %, le PFU l'emporte systématiquement ; entre les deux, la composition des revenus décide, l'abattement sur dividendes pouvant renverser le calcul. La leçon assortit ce tableau d'une réserve nette : **orientation pédagogique, pas conseil personnalisé** — le calcul exact dépend de la situation complète, et relève d'un professionnel agréé.

## Notes liées

- **⬅️ Précédente** · [[2026-08-08_lecon-placement-financier_09_frais-impact-long-terme]]
  la leçon l'énonce elle-même : les frais et la fiscalité sont les deux leviers d'érosion silencieuse du rendement. Le premier se négocie, le second s'optimise par le choix de l'enveloppe — les deux se cumulent
- **🔗 Pont** · [[2026-07-11_lecon-placement-financier_05_enveloppes-francaises]]
  cette leçon donne enfin le chiffrage de ce que la leçon 05 posait en principe : une même somme sur les mêmes actifs ne rend pas pareil selon le contenant, précisément à cause de ces régimes dérogatoires
- **🔗 Pont** · [[2026-07-25_lecon-placement-financier_07_per-preparation-retraite-fiscalite]]
  la TMI commande dans les deux cas, mais en sens inverse : elle décide ici du choix PFU/barème sur des revenus perçus, et là de la valeur de la déduction à l'entrée. Une TMI élevée pousse au PFU et rend le PER plus attractif
- **🔗 Pont** · [[2026-07-18_lecon-placement-financier_06_assurance-vie-fonds-euros-uc-fiscalite]]
  le seuil des huit ans y prenait tout son sens sans être comparé au droit commun ; c'est ce régime à 31,4 % qui sert de point de référence
- **🔗 Pont** · [[2026-08-09_lecon-revenus-passifs_03_trois-intrants-point-bascule]]
  le point de bascule d'un modèle passif se calcule sur des revenus nets. Sans intégrer 31,4 % de prélèvements, la date de rentabilité annoncée est fausse d'environ un tiers
