...
**Seconde passe du 25/09/2026 — deux messages de plus dans le même tableau.** ① *« TS2591: Cannot find name '__dirname' »* : reproduit avec **TypeScript 7.0.2**, la version du projet, dans **quatre configurations** — avec et sans `"types": []`, en `commonjs` et en `nodenext`, avec et sans `node_modules`. Le compilateur rend à chaque fois **`error TS2304`**, jamais TS2591. La variante TS2591 existe (c'est celle qui suggère d'installer `@types/node`) mais n'a pas pu être déclenchée, et son catalogue n'est plus greppable depuis que `tsc` est un binaire natif : ce qui est écrit est donc ce qui a été mesuré, pas ce qu'on aurait aimé mesurer. La cause et la correction, elles, étaient justes. ② *« node: command not found »* est la forme **bash** amputée de son préfixe — symétrique exact du défaut de la leçon 01 sur `npm`. Et une sortie de ce document s'est révélée **exacte au mot près** : le bloc `error TS2741: Property 'recents' is missing in type '{ nombre: number; taille_ko: number; }' but required in type 'Categorie'`, rejoué sur un cas minimal — seul le préfixe `fichier(ligne,colonne)` manquait, la leçon ayant coupé la ligne en trois pour la mise en page. À savoir : `Categorie` a été scindé en `CategorieResumee` et `ReponseLivrables` à la leçon 08, donc le message d'aujourd'hui nommerait un autre type.
