...
**Relue le 25/09/2026 — et c'est la seule leçon du parcours qui avait un message exact.** `Error: listen EADDRINUSE: address already in use :::3000` sort au caractère près : **un message juste sur les vingt relevés dans les sept leçons reprises ce jour**, et la leçon 04 le republiera tronqué deux semaines plus tard. Les trois autres ne tenaient pas. `ReferenceError: require is not defined` était coupé avant le remède que Node imprime lui-même — `in ES module scope, you can use import instead` : la moitié supprimée était la moitié utile. `fetch: TypeError: Failed to fetch` portait un préfixe `fetch: ` inventé ; mesuré dans un navigateur sur un port fermé, l'exception est `TypeError: Failed to fetch` et la console imprime, séparément, `Failed to load resource: net::ERR_CONNECTION_REFUSED`. Et `Error: Cannot find module 'node:http'` décrit un Node antérieur à v14.18.0 qu'on **n'a pas sous la main** : c'est maintenant écrit dans la ligne, avec le message voisin qui, lui, se reproduit. La sortie de démarrage `# Tu dois voir : ✅ Portail Livrables démarré` a été vérifiée au commit `a9eb6a4` : le serveur du 07/08 imprimait bien ce ✅, disparu du code depuis — troisième sortie du parcours exacte à sa date et périmée ensuite.
