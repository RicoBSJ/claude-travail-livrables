...
**Seconde passe du 25/09/2026 — le cinquième message du tableau.** *« ENOENT: no such file or directory »* perdait son préfixe `Error:` **et sa fin**. Reproduit : `readdirSync` sur un dossier absent rend `Error: ENOENT: no such file or directory, scandir '/tmp/nexistepas_xyz'` — Node nomme l'**appel système** (`scandir` pour un dossier, `open` pour un fichier) **et le chemin exact**, c'est-à-dire les deux seules informations qui disent quoi corriger, et les deux qui manquaient. **Et une ligne de cette leçon mérite d'être relevée pour elle-même** : la check-list annonce *« node scripts/serveur.js → le terminal affiche : Portail Livrables démarré → http://localhost:3000 »* — **exact**, au commit de cette leçon comme aujourd'hui. C'est la leçon 06, deux semaines plus tard, qui écrira *« Serveur démarré sur … »*, jamais imprimé par rien. Une leçon peut désapprendre ce qu'une autre avait mesuré.
