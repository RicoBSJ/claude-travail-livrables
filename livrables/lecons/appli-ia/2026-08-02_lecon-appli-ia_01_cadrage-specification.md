...
**Seconde passe du 25/09/2026 — le troisième message du tableau, que la première n'avait pas vu.** *« command not found: npm »* n'est pas une sortie : c'est la moitié d'une sortie **zsh**. Reproduit dans les deux shells : zsh rend `zsh:1: command not found: npm`, bash rend `bash: npm: command not found` — même information, **ordre des mots inversé**. La règle 9 du parcours impose depuis le 11/09 de préciser le shell quand une commande est donnée ; personne n'avait songé qu'elle vaut aussi pour les messages qu'il renvoie. En revanche *« 0 fichier(s) partout »* est de la bonne forme — `node scripts/inventaire.js` imprime bien « lecons 307 fichier(s) 3629 Ko » — et sa cause (RACINE mal pointée) est la bonne.
