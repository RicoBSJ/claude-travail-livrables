...
**Et cette raison est un défaut structurel du parcours : les deux sources scientifiques répondent 200 en servant un mur reCAPTCHA.** `curl` avec en-tête navigateur sur les deux articles PMC renvoie **168 caractères** : *« Checking your browser — reCAPTCHA »*. Le test de liens que le prompt prescrit — un `curl` qui relève le code HTTP — voit donc un **200 franc** sur une page qui ne contient pas l'article. La leçon écrit *« toutes vérifiées le 08/09/2026 »* et *« effectivement consultées »* : ce peut être vrai via WebFetch, qui est un autre client, mais **la vérification annoncée n'est pas reproductible par le contrôle du prompt**. C'est exactement le cas qui a donné la règle B3 à `psychopathologie` — un HTTP 200 de `has-sante.fr` ne prouve pas qu'on a la page. `hypnose-lecon`, avec ses 7 règles, n'a pas d'équivalent. La voie de contournement existe et fonctionne : l'**API Europe PMC**, qui rend titre, auteurs, année, DOI et résumé en JSON, sans blocage.
