...
- Modèle forcé : **sonnet** · **plafond de coût par-job** — `run_job.sh` est la **source de vérité** (bloc `case "$JOB_ID"`), ce tableau n'en est que le reflet :
  ...
  | **3,50 $** | `imac-veille` |
  | **3 $** | `ai-act-veille` · `astrologie-karmique-lecon` · `dzogchen-lecon` · `enneagramme-lecon` · `hypnose-lecon` · `serafin-ph-veille` · `stoicisme-lecon` |
  | **2 $** (défaut) | les 4 autres : `appli-ia-lecon` · `placement-financier-lecon` · `psychopathologie-lecon` · `revenus-passifs-lecon` |
