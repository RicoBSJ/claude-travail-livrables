...
- **Les contrôles communs** (`outils/scripts/controle_attributions.py`, `controle_decompte.py`) sont appelés par les prompts, jamais copiés. **Toute retouche se rejoue avec `outils/scripts/non_regression.sh`** (90 s, diff des verdicts sur les 221 documents) avant d'être poussée — le hook `outils/scripts/hooks/pre-push` (`git config core.hooksPath outils/scripts/hooks`, une fois par clone) le lance et **refuse le push** si un verdict a changé. Les pushs des jobs passent par le même hook en mode `--docs` (quelques secondes) : un livrable ou une veille qui régresse, ou un document nouveau qui bloque, **n'est pas publié** — le log du job dit pourquoi.
