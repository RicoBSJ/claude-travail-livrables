...
case "$JOB_ID" in
  ...
  psychopathologie-lecon) BUDGET="3.00" ;;  # relevé le 06/09/2026, en même temps que le durcissement du même jour : il ajoute 6 règles portant leurs incidents propres (A3 régime de l'énoncé clinique, B2 hiérarchie des sources, B3 soft-200 de has-sante.fr, C2 grille de la leçon, D4 double datation ANESM/HAS) et 4 contrôles mécaniques avant génération. Le prompt passe de 15 231 à 24 226 caractères, et le grep de B3 s'ajoute au curl de B1 sur chaque page HAS. Dernier job encore au plafond par défaut ; le laisser à 2 $ après ce durcissement revenait à programmer l'échec du créneau du lundi
